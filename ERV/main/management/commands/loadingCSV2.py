    ...
    class Command(BaseCommand):
        ...
        def handle(self, *args, **kwargs):
            for i in range(len(surnames)):
                ...
                if isProcessed(names[i], surnames[i], row_date[i]):
                    continue
                ...
                if ErvExists(worker, row_date[i]):
                    ...
                    erv = ERV.objects.get(current_date = datetime.strptime(row_date[i], FORMAT_DATE), worker = Worker.objects.get(name__iexact = names[i], surname__iexact = surnames[i]))
                    if erv.exit_time and not erv.enter_time and timeDifference(row_date[i], erv.exit_time, datetime.strptime(times[i], FORMAT_TIME)) > datetime.strptime('00:02:00', FORMAT_TIME):
                    ...
                    if datetime.strptime(times[i], FORMAT_TIME) <= datetime.strptime('12:00:00', FORMAT_TIME):
                        ERV(worker = worker, current_date = datetime.strptime(row_date[i], FORMAT_DATE), enter_time = datetime.strptime(times[i], FORMAT_TIME), flag = 'redovni rad').save()
                    else:
                        ERV(worker = worker, current_date = datetime.strptime(row_date[i], FORMAT_DATE), exit_time = datetime.strptime(times[i], FORMAT_TIME), flag = 'redovni rad').save()
            
            for i in range(len(surnames)):
                worker = Worker.objects.get(name__iexact = names[i], surname__iexact = surnames[i])
                erv = ERV.objects.get(current_date = datetime.strptime(row_date[i], FORMAT_DATE), worker = worker)
                erv.processed = True
                erv.save()
